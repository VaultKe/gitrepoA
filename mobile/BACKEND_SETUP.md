...
1. **Try the real backend first** at `http://localhost:8085/api/v1`
...
1. Make sure your Golang backend is running on port 8085
...
1. Check that your backend is running on `http://localhost:8085`
2. Verify CORS middleware is properly configured
3. Check browser developer tools for specific CORS error messages
4. Try accessing the API directly in browser: `http://localhost:8085/api/v1/marketplace/products`

#### Backend not starting?

1. Check if port 8085 is already in use: `lsof -i :8085`
...
const API_BASE_URL = process.env.REACT_APP_API_URL || 
  (__DEV__ ? 'http://localhost:8085/api/v1' : 'https://api.vaultke.com/api/v1');
```
...
curl -X OPTIONS http://localhost:8085/api/v1/marketplace/products \
